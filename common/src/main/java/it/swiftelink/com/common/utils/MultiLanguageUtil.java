...
public class MultiLanguageUtil {
    ...
    private Locale getLanguageLocale() {
        int languageType = Application.getSPUtils().getInt(MultiLanguageUtil.SAVE_LANGUAGE, 0);
    ...
    public void updateLanguage(int languageType) {
        Application.getSPUtils().put(MultiLanguageUtil.SAVE_LANGUAGE, languageType);
        MultiLanguageUtil.getInstance().setConfiguration();
        EventBus.getDefault().post(new OnChangeLanguageEvent(languageType));
        switch (languageType) {
            case LanguageType.LANGUAGE_EN:
                Application.getSPUtils().put("appLanguage", Constants.EN_US);
                break;
            case LanguageType.LANGUAGE_CHINESE_SIMPLIFIED:
                Application.getSPUtils().put("appLanguage", Constants.ZH_CN);
                break;
            case LanguageType.LANGUAGE_CHINESE_TRADITIONAL:
                Application.getSPUtils().put("appLanguage", Constants.ZH_TW);
                break;
        }
    }
    ...
    public int getLanguageType() {
        int languageType = Application.getSPUtils().getInt(MultiLanguageUtil.SAVE_LANGUAGE, LanguageType.LANGUAGE_FOLLOW_SYSTEM);
        if (languageType == LanguageType.LANGUAGE_CHINESE_SIMPLIFIED) {
            ...
            } else if (locale.getLanguage().endsWith("tw")) {
                return LanguageType.LANGUAGE_CHINESE_TRADITIONAL;
            }
        }
